...
cel = ballgown(dataDir = "BristolN_EN_WB235", samplePattern = "", pData=pheno_data)
...
subset(cnts.ga,gene_name=='lbp-8')

#Test genes
rownames(cnts.ga)<-cnts.ga$t_name
dim(subset(cnts.ga,is.na(t_id)))
dim(subset(cnts.ga,is.na(gene_id)))
dim(subset(cnts.ga,is.na(gene_name)))
...
#Case check
subset(full_table,t_name=='transcript:Y49F6B.11')
subset(cnts.gae,t_name=='transcript:Y49F6B.11')
subset(cnts.gae,gene_name=='lbp-8')
...
head(subset(unq,Unique==FALSE))
subset(cnts.gae,gene_id=='MSTRG.10002')
...
#Top expressed
#head(cnts.gae,n=100)
subset(cnts.gae,t_name=='transcript:F31E3.5.1')
subset(cnts.gae,gene_id=='MSTRG.6868')


subset(cnts.gaf,gene_name=='lbp-8')
...
egTRANS<-toTable(org.Ce.egENSEMBLTRANS2EG)
#Missing Transcript to EntrezGene
dim(subset(egTRANS,is.na(gene_id) | is.na(trans_id)))
mat<-match(gsub('transcript:','',cnts.gaf$t_name),egTRANS$trans_id)
cnts.gaf$EntrezGene<-egTRANS$gene_id[mat]


egENSEMBL<-toTable(org.Ce.egENSEMBL2EG)
#Missing ENSEMBL to EntrezGene
dim(subset(egENSEMBL,is.na(gene_id) | is.na(ensembl_id)))
me<-match(gsub('gene:','',cnts.gaf$gene_id),egENSEMBL$ensembl_id)
cnts.gaf$EntrezGene<-ifelse(is.na(cnts.gaf$EntrezGene),egENSEMBL$gene_id[me],cnts.gaf$EntrezGene)

me2<-match(cnts.gaf$gene_id,egENSEMBL$ensembl_id)
cnts.gaf$EntrezGene<-ifelse(is.na(cnts.gaf$EntrezGene),egENSEMBL$gene_id[me2],cnts.gaf$EntrezGene)


egWB<-toTable(org.Ce.egWORMBASE)
#Missing Wormbase to EntrezGene
dim(subset(egWB,is.na(gene_id) | is.na(wormbase_id)))
mew<-match(gsub('gene:','',cnts.gaf$gene_id),egWB$wormbase_id)
cnts.gaf$EntrezGene<-ifelse(is.na(cnts.gaf$EntrezGene),egWB$gene_id[mew],cnts.gaf$EntrezGene)

egALIAS<-toTable(org.Ce.egALIAS2EG)
#Missing ALIAS to EntrezGene
dim(subset(egALIAS,is.na(gene_id) | is.na(alias_symbol)))
mal<-match(cnts.gaf$gene_name,egALIAS$alias_symbol)
cnts.gaf$EntrezGene<-ifelse(is.na(cnts.gaf$EntrezGene),egALIAS$gene_id[mal],cnts.gaf$EntrezGene)

egSYMBOL<-toTable(org.Ce.egSYMBOL2EG)
#Missing SYMBOL to EntrezGene
dim(subset(egSYMBOL,is.na(gene_id) | is.na(symbol)))
ms<-match(cnts.gaf$gene_name,egSYMBOL$symbol)
cnts.gaf$EntrezGene<-ifelse(is.na(cnts.gaf$EntrezGene),egSYMBOL$gene_id[ms],cnts.gaf$EntrezGene)

egENSEMBLP<-toTable(org.Ce.egENSEMBLPROT2EG)
mENP<-match(cnts.gaf$gene_name,egENSEMBLP$prot_id)
cnts.gaf$EntrezGene<-ifelse(is.na(cnts.gaf$EntrezGene),egENSEMBLP$gene_id[mENP],cnts.gaf$EntrezGene)
...
mEMP<-match(cnts.gaf$EntrezGene,egENSEMBLP$gene_id)
cnts.gaf$EntrezProt<-egENSEMBLP$prot_id[mEMP]

#head(toTable(org.Ce.egUNIPROT))

#head(toTable(org.Ce.egWORMBASE))



#head(cnts.gaf)
subset(cnts.gaf,gene_name=='F44G3.2')

#Missing annotations
missing<-subset(cnts.gaf,is.na(EntrezGene) & gene_name!='.')[,c('t_id','t_name','gene_id','gene_name','EntrezGene','EntrezProt','Symbol')]
dim(missing)
#265
#missing

dots<-subset(cnts.gaf,is.na(EntrezGene) & gene_name=='.')[,c('t_id','t_name','gene_id','gene_name','EntrezGene','EntrezProt','Symbol')]

dim(dots)
#1023

#None can be found
subset(missing,gene_name=='F44G3.2')
subset(egTRANS,trans_id %in% missing$gene_name )
subset(egENSEMBLP,prot_id %in% missing$gene_name )
...
colorder<-c('t_name','t_id','EntrezGene','EntrezProt','Symbol','gene_id','gene_name',
...
cnts.gae.w<-cnts.gae[,setdiff(colorder,c('EntrezGene','EntrezProt','Symbol'))]
...
gene.info<-cnts.gaf[,c('t_id','gene_id','EntrezGene','EntrezProt','Symbol','gene_name','Unique','chr','strand','start','end','num_exons','length')]
gene.data<-cnts.gaf[,codes]
...
#Or choose level
#samplethreshold<-8
#setwd("~/Projects/2015-Metformin/Sequencing/CElegans")
#odir<-paste('BristolN_EN_WB235_Results/R',Rversion,'_Above_',samplethreshold,'_PDF-',priordf,'_PN-',priorn,sep='')
#odir<-paste('BristolN_EN_WB235_Results/R',Rversion,'_Above_',samplethreshold,'_Manual_Testing',sep='')
#print(odir)
#dir.create(odir, showWarnings = TRUE, recursive = FALSE, mode = "0777")
...
keep<- rowSums(cpm(dt)[,grep('^C[[:digit:]]',colnames(dt))]>1)==4 |
  rowSums(cpm(dt)[,grep('^M[[:digit:]]',colnames(dt))]>1)==4 |
  rowSums(cpm(dt)[,grep('^R[[:digit:]]',colnames(dt))]>1)==4 | 
  rowSums(cpm(dt)[,grep('^RM[[:digit:]]',colnames(dt))]>1)==4
...
dim(d)
#22501->12046
#22501->11422 threshold 8 any
#22501->11184 threshold 4 consistent
...
batch.lvl<-c('1','1','2','2',
             ...
             '1','2','2','2',
             '1','1','2','2')
design.mat <- model.matrix(~ 0 + d$samples$group+batch.lvl) #
...
# residual.df=n.samples-n.groups
# residual.df=16-4=12
# prior.n=10
# prior.df=residual.df*prior.n
# prior.df=12*10=120
# prior.n<-getPriorN(d2, prior.df=10)
...
# } else {
#   print('For R 3.3')
#   d2<-estimateDisp(d,design.mat,prior.df=110)#,trend.method='loess' ,prior.df=120
# }

# auto.n<-getPriorN(d,design = design.mat)
# d2<-estimateDisp(d,design.mat,prior.n = auto.n)

# 
# prior.n=10/11
# #10/11
# 
# #No of samples-no of groups
# residual.df=3.850181/0.35
# 
# 
# prior.df=getPriorN(d2,design = design.mat)*residual.df
# prior.df
# 
# getPriorN(d,design = design.mat)

# residual.df
# 3.850181/0.3500165

#Different ways of doing dispersion analysis
# if (priordf=='Old') {
#   print('Using explicit dispersion estimation')
#   d2 <- estimateGLMCommonDisp(d,design.mat)
#   d2 <- estimateGLMTrendedDisp(d2,design.mat, method="power")
#   d2 <- estimateGLMTagwiseDisp(d2,design.mat)
#   print(paste('Prior.n=',d2$prior.n,' Residual.df=',d2$prior.df/d2$prior.n,' Prior.df=',d2$prior.df,sep=''))
# } else if (priordf=='Auto') {
#   print('Using automatic default dispersion estimation')
#   d2<-estimateDisp(d,design.mat)
#   print(paste('Prior.n=',d2$prior.n,' Residual.df=',d2$prior.df/d2$prior.n,' Prior.df=',d2$prior.df,sep=''))
# } else if (priordf=='AutoDF') {
#   print('Using automatic guided dispersion estimation')
#   priorn<-getPriorN(d,design = design.mat)
#   residualdf=16-5
#   priordf.calc<-priorn*residualdf
#   d2<-estimateDisp(d,design.mat,prior.df = priordf.calc)
#   print(paste('prior.n=',priorn,' residual.df=',residualdf,' prior.df=',priordf.calc,sep=''))
# } else if (priordf=='AutoDF') {
#   print('Using manual dispersion estimation')
# #   priorn<-getPriorN(d,design = design.mat)
#   residualdf=16-5
#   priordf.calc<-priorn*residualdf
#   print(paste('prior.n=',priorn,' residual.df=',residualdf,' prior.df=',priordf.calc,sep=''))
#   d2<-estimateDisp(d,design.mat,prior.n=priorn)
# } else {  
...
comparisons<-c('SM-S','RM-R','R-S','T-C_general','R-S_general','S-R_general','SM-S-(RM-R)')
oldids<-c('M-C','RM-R','R-C','T-C_general','R-C_general','C-R_general','M-C-(RM-R)')
...
for (cid in 1:nrow(allcomparisons)) {
  ...
  for (thre in c('None','1FC')) {
    ...
    result<-as.data.frame(topTags(lrt,n=dim(d$genes)[1]))
    ...
    if (summary(de.glm)[3]>0 | summary(de.glm)[1] > 0) {
...
for (thres in c('None','1FC')) {
  ...
  #Melt
  all.results.rm<-melt(all.results.r,id.vars = c('Comparison','gene_id','gene_name','EntrezGene','EntrezProt','Symbol',
  ...
  all.results.rcp<-dcast(all.results.rmc,t_id+gene_name+EntrezGene+EntrezProt+Unique+Pathways+GO_terms+
