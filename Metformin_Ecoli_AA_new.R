...
col.names<-read.xlsx2('Ecoli_old_AA.xlsx',sheetName='AA',
                        header=FALSE,endRow=1,
                        colClasses=rep("character",30))
...
#-match(missing.cols,colnames(met.raw))
met.raw.c<-met.raw[-5,]



metabolites<-colnames(met.raw.c)[7:length(colnames(met.raw.c))]
metabolites
...
contrasts<-read.contrasts('!Contrasts_Ecoli_AA_metabolomics.xlsx','Contrasts_values',sel.groups)
