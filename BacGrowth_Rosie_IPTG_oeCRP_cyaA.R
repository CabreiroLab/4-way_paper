...
mutdata <- read_csv("Resistance_mutants_growth_assays/2017-Rosie_Mutants/Summary.csv") %>%
  filter(Strain %in% c("crp", "OP50-C") & Metformin_mM == 0) %>%
  mutate(
    Overexpression = "None",
    IPTG_mM = 0
  ) %>%
...
data.sum <- data %>%
  group_by(Measure, Normalisation, Strain, Overexpression, IPTG_mM) %>%
  summarise(
    Mean = mean(Value),
    SD = sd(Value),
    SE = SD / sqrt(n())
...
data_ts <- read_csv("IPTG titration/Data/Data.csv") %>%
  bind_rows(mutdatats) %>%
  filter(!is.na(Strain)) %>%
  filter(Data == "600nm_f") %>%
  filter(!(Strain == "cyaA" & Overexpression == "None" & Replicate == 3)) %>%
...
data_ts %>%
  group_by(Strain, Overexpression, IPTG_mM) %>%
  summarise(Count = n()) %>%
  View()


IPTGcols <- colorRampPalette(c("red", "blue4"))(4)
names(IPTGcols) <- levels(data_ts$IPTG_mM)
IPTGlab <- "IPTG,\nmM"

ggplot(data_ts, aes(x = Time_h, y = OD, color = IPTG_mM)) +
  geom_line(aes(group = interaction(Plate, Replicate, TReplicate, IPTG_mM))) +
  xlab("Time, h") +
  scale_colour_manual(name = IPTGlab, values = IPTGcols) +
  scale_x_continuous(breaks = seq(0, 24, by = 6)) +
  facet_grid(Overexpression ~ Strain, labeller = label_both)
...
datats.sum <- data_ts %>%
  group_by(Strain, Overexpression, IPTG_mM, Time_h) %>%
  summarise(
    OD_Mean = mean(OD),
    OD_SD = sd(OD)
  )
...
stats.S <- data %>%
  ...
  group_by(Measure,Overexpression,IPTG_mM) %>%
  do(tidy(lm(Value ~ Strain, data = .))) %>%
  mutate(
    Comparison = case_when(
      str_detect(term, "\\:") ~ "Interaction",
...
stats <- stats.O %>%
  ...
  select(Measure, Comparison, Strain, Overexpression, IPTG_mM, everything(), -term)
...
data.sum %>%
  ...
  geom_text(
    data = stats %>%
      filter(Measure == "logAUC" & Strain !="cyaA" & Comparison == "Interaction_Overexpression-IPTG_mM"),
...
ggsave(
  file = paste0(odir, "/Growth_Comparison_vs_OP50-C_Control_crp.pdf"),
...
ggsave(
  file = paste0(odir, "/Growth_Comparison_vs_OP50-C_Control_cyaA.pdf"),
  width = 55, height = 41, units = "mm", scale = 2, device = cairo_pdf, family = "Arial"
)
