...
met.raw<-read.xlsx2('Metformin AA worm data.xlsx',sheetName='Complete_table',
                    header=TRUE,endRow=55,
                    colClasses=c(rep("character",7), rep("numeric", 19)))



head(met.raw)

AAs<-colnames(met.raw)[8:length(colnames(met.raw))]
...
met$Drug_mM<-as.factor(met$Drug_mM)
...
metslm<-dcast(met,Sample+Group+Strain+Drug_mM+Replicate~Metabolite,value.var = c('logConc'),drop=TRUE)#,fill = as.numeric(NA)

metslm<-dcast(subset(met,Strain %in% c('OP50','OP50-MR')),Sample+Group+Strain+Drug_mM+Replicate~Metabolite,value.var = c('logConc'),drop=TRUE)#,fill = as.numeric(NA)
...
pcadata[,c('Sample','Strain','Group','Drug_mM')]<-metslm[,c('Sample','Strain','Group','Drug_mM')]
...
ellipses<-ddply(pcadata,.(Strain,Group,Drug_mM), summarise, x=getellipse(PC1,PC2,1)$x,y=getellipse(PC1,PC2,0.75)$y ) 
...
ggplot(met.clean,aes(x=Metabolite,y=logConc,color=Drug_mM))+
...
ggplot(met.clean,aes(x=Metabolite,y=Conc,color=Drug_mM))+
  ggtitle('Comparison between Control and Treatment. Boxplot: +/-SD, Min/Max')+
  stat_summary(fun.data=MinMeanSDMMax, geom="boxplot",position = "identity") +
  geom_point()+
  geom_text(aes(label=Replicate),color='black',size=2)+
...
summary<-ddply(met.mes,.(Metabolite,Strain,Drug_mM,Group,Measure),summarise,Mean=mean(Value),SD=sd(Value))
...
sum.c<-dcast(sum.m,Metabolite+Strain+Drug_mM+Group+Index~Measure+Stat,value.var = 'Value',drop = TRUE)
...
dev.copy2pdf(device=cairo_pdf,
             ...
             width=5,height=30, useDingbats=FALSE)
...
strainlist<-c('OP50','OP50-MR','OP50-crp','nhr-49','BW52113')
