...
col.names<-read.xlsx2('Ecoli_100met_data.xlsx',sheetName='Export',
                    header=FALSE,endRow=1,startRow = 1,
                    colClasses=rep("character",109))

met.raw<-read.xlsx2('Ecoli_100met_data.xlsx',sheetName='Export',
           header=FALSE,endRow=21,startRow = 2,
           colClasses=c(rep("character",7), rep("numeric", 102)))
colnames(met.raw)<-t(col.names)[,1]


norm<-read.xlsx2('Ecoli_100met_normalisation.xlsx',sheetName='Normalisation',
                    header=TRUE,endRow=21,
...
miss.cols<-apply(met.raw, 2, function(x) any(is.na(x)))
miss.rows<-apply(met.raw, 1, function(x) any(is.na(x)))
...
met.raw.c<-met.raw[,-match(missing.cols,colnames(met.raw))]



colnames(met.raw.c)
metabolites<-colnames(met.raw.c)[8:length(colnames(met.raw.c))]


met.norm<-met.raw.c


#Normalisation by protein content
met.norm[,metabolites]<-met.norm[,metabolites]/norm[match(as.character(met.raw.c$ID2),as.character(norm$Sample)),rep('Protein..ug.ml.',length(metabolites))]


head(met.norm)


#met.norm[,metabolites]<-met.norm[,metabolites]/met.norm$Protein..ug.ml.


#Additional normalisation
rowmeans<-apply(met.norm[,metabolites],1,mean)
norms<-rowmeans/mean(rowmeans,na.rm = TRUE)

met.norm[,metabolites]<-met.norm[,metabolites]/norms



head(met.norm)
#Export for Kaleta
met.kal<-subset(met.norm,!Sample %in% c('MR_T_2','MR_C_3') & Strain!='OP50-P5')
met.kal$ID<-NULL
met.kal$ID2<-NULL


write.csv(met.norm,paste(odir,'/Raw_data.csv',sep=''),row.names = FALSE)
write.csv(met.kal,paste(odir,'/Raw_data_Kaleta.csv',sep=''),row.names = FALSE)





met<-melt(met.norm,measure.vars = metabolites,variable.name = 'Metabolite',value.name = 'Conc')
...
strains<-c('OP50','OP50-MR','OP50-P5')
...
met<-subset(met,!Sample %in% c('MR_T_2','MR_C_3') ) #,'P_C_1' 
...
ellipses<-ddply(pcadata,.(Strain,Group,Metformin_mM), summarise, x=getellipse(PC1,PC2,1)$x,y=getellipse(PC1,PC2,1)$y ) 
...
contrasts<-read.contrasts('!Contrasts_Ecoli_100met_metabolomics.xlsx','Contrasts_values',sel.groups)
contrasts.table<-contrasts$Contrasts.table
contr.matrix<-contrasts$Contrasts.matrix

strainlist<-c('OP50','OP50-MR','OP50-P5')
...
comparisons<-c("Treatment effect on OP50","Treatment effect on OP50-MR",
               "Mutant difference for OP50-MR","Mutant difference for OP50-P5","Interaction with treatment for OP50-MR")
...
dev.copy2pdf(device=cairo_pdf,file=paste(odir,'/Comparison_heatmap.pdf',sep = ''),
             width=6,height=18,useDingbats=FALSE)




longevity<-subset(results.castfull,`C_T-C_C_FDR`<0.05 & `P_C-C_C_FDR`<0.05 & `MR_T-MR_C_FDR`>0.05 & `MR_C-C_C_FDR`>0.05)$Metabolite
